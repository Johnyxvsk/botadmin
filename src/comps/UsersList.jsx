...
const UsersList = ({ bikerData, isFetching }) => {
  const [showFilter, setshowFilter] = useState(false)
  const [show, setFilter] = useState(false)
  const [bikers, setBikers] = useState(bikerData)
  ...
  const setFilterCheck = (e) => {
    ...
    if (list.includes(opt)) {
      ...
      if (index > -1) {
        list.splice(index, 1)
        let newtest = bikerData.filter((item, idx) => {
          ...
          components={{
            Toolbar: (props) => (
              <div className="MaterialTableToolbar">
                ...
                <div className="toolbarPesquisa toolItem">
                  <i
                    className="loading material-icons-outlined"
                    >
                    loading
                  </i>
                </div>
